...
class Config:
    def __init__(self):
        ...
        if not self.base_url:
            raise ValueError("API_BASE_URL is not set in environment variables or settings.yaml")

        if not self.token:
            raise ValueError("API_TOKEN is not set in environment variables or settings.yaml")
